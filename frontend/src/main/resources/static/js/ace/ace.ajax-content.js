;(function($ , undefined){
	...
	function AceAjax(contentArea,settings) {
		...
		this.getUrl = function(url, hash, manual_trigger, method, data, updateView, extraParams) {
			...
			$.ajax(ajax_params).error(function(response,textStatus,errorThrown){
				$contentArea.trigger('ajaxloaderror', {url: url, hash: hash, method: method, data: data});
				self.stopLoading(true);
                var errorStatus = response.status;
                if(errorStatus === 404){
                    layerFn.alert("页面不存在(无权限操作)",AppKey.code.code199);
                }else if(errorStatus === 0){
                    layerFn.connectFailure();
                }else{
                    layerFn.alert("未知错误,请联系管理员",AppKey.code.code204);
                }
                return;
			}).done(function(result,textStatus,errorThrown){
                var not_login_in = layerFn.checkLogin(result);
                if(not_login_in){
                    return;
                };
                try {
                    if(typeof JSON.parse(result) == "object"){
                        ...
                        layerFn.closeEvent(json.msg,json.code,function(){
				...
				if(typeof self.settings.update_active === 'function') {
					...
					link_element = $('a[data-url="'+hash+'"]');//菜单定位
				...
				if(updateView) {
                    ...
                    $contentArea.empty().html(result);//清空后再赋值
				}
				...
				if( !manual_trigger && updateView ) {
					$('html,body').animate({scrollTop: 0}, 250);//显示动画
				}
		...
		this.stopLoading = function(stopNow){
            ...
            layerFn.closeIndex(self.layIndex);
		}
		...
		function updateTitle(link_text) {
			...
			if($title.length > 0) {
				document.title = $title.text();// 赋值给title标题
				$title.remove();
			}
