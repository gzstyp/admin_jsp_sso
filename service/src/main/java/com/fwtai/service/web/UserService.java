...
public class UserService{
    ...
    public List<HashMap<String,String>> queryPermissions(){
        return userDao.queryPermissions();
    }

    public List<HashMap<String,String>> getPermissions(final String userId){
    ...
    public String login(final HttpServletRequest request){
        ...
        final String validate = ToolClient.validateField(params,new String[]{user_name,user_password});
        if(validate != null)return validate;
        final String userName = params.get(user_name);
        final String userPassword = params.get(user_password);
        final Subject subject = SecurityUtils.getSubject();
        final UsernamePasswordToken token = new UsernamePasswordToken(userName,ToolSHA.encoder(userPassword,userName));
        try {
            subject.login(token);
            final HashMap<String,Object> map = (HashMap<String,Object>) subject.getPrincipals().getPrimaryPrincipal();
            final String userId = (String) map.get(ConfigFile.LOGIN_KEY);
            map.remove(ConfigFile.LOGIN_KEY);
            ...
            return ToolClient.createJsonFail("登录失败,稍候重试");
        }
    }
