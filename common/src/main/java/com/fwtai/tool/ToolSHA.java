...
public final class ToolSHA{
    ...
    /*私盐加密*/
    private final static String PRIVATESALT = "fwt.cloud$Yinlz.Com";

    /**
     * 使用Sha256Hash加密一次生成64的字符串作为密盐
     * @作者 田应平
     * @param userName 密盐
    ...
    */
    private final static String encryptHash(final String userName){
        return String.valueOf(new Sha256Hash(userName,PRIVATESALT));
    }

    /**
     * 使用Sha256Hash对用户名和私盐加密生成64个密盐,又使用SHA-1方式用户名与64位密盐加密
     * @作者 田应平
     * @param password 密码
     * @param userName 密盐
    ...
    */
    public final static String encoder(final String password,final String userName){
        return String.valueOf(new SimpleHash(SHA1,password,encryptHash(userName)));
    }
